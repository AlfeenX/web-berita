<x-layouts::app :title="__('Manage Kategori')">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Manage Kategori</h1>
            <p class="text-zinc-500 dark:text-zinc-400 mt-1">Buat, perbarui, dan hapus kategori artikel Anda secara langsung.</p>
        </div>
    </div>
    ...
    <div class="mb-8">
        <flux:card class="p-6 shadow-md border-zinc-200/80 dark:border-zinc-700/80 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-md">
            <div class="mb-5">
                <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Tambah Kategori Baru</h2>
                <p class="text-sm text-zinc-500 dark:text-zinc-400">Kategori baru akan langsung dapat digunakan oleh penulis untuk artikel mereka.</p>
            </div>

            <form action="{{ route('categories.store') }}" method="POST" class="flex flex-col sm:flex-row gap-4 items-end max-w-3xl">
                @csrf
                <flux:field class="grow w-full">
                    <flux:label class="text-zinc-600 dark:text-zinc-300 font-medium">Nama Kategori</flux:label>
                    <flux:input 
                        name="name" 
    ...
    <flux:card class="p-6 shadow-md border-zinc-200/80 dark:border-zinc-700/80 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-md">
        ...
        @if ($categories->isEmpty())
            <div class="flex flex-col items-center justify-center py-12 text-zinc-500 dark:text-zinc-400">
                ...
                <p class="text-base font-medium">Belum ada kategori</p>
                <p class="text-sm text-zinc-400">Gunakan form di atas untuk membuat kategori pertama Anda.</p>
            </div>
        @else
            <flux:table>
                <flux:table.columns>
                        ...
                        <flux:table.row x-data="{ editing: false, name: '{{ addslashes($category->name) }}' }" class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors duration-150">
                            ...
                            <flux:table.cell>
                                <template x-if="!editing">
                                    <span class="font-semibold text-zinc-800 dark:text-zinc-200">{{ $category->name }}</span>
                                </template>
                                <template x-if="editing">
                                    <form id="edit-form-{{ $category->id }}" action="{{ route('categories.update', $category) }}" method="POST" class="flex gap-2 items-center w-full max-w-md">
                                        @csrf
                                        @method('PUT')
                            ...
                            <flux:table.cell class="text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold rounded-full {{ $category->articles()->count() > 0 ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400' }}">
                            ...
                            <flux:table.cell align="end">
                                    ...
                                    <template x-if="editing">
                                        <div class="flex gap-1.5">
                                            ...
                                            <flux:button 
                                                size="sm" 
                                                variant="ghost" 
                                                @click="editing = false; name = '{{ addslashes($category->name) }}'"
                                            >
                                                Batal
                                            </flux:button>
                                        </div>
                                ...
                                {{-- Modal Hapus --}}
                                <flux:modal name="delete-category-{{ $category->id }}" class="max-w-md">
                                    <form action="{{ route('categories.destroy', $category) }}" method="POST" class="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <flux:subheading class="mt-2 text-zinc-500 dark:text-zinc-400 leading-relaxed">
                                                Apakah Anda yakin ingin menghapus kategori <strong class="text-zinc-900 dark:text-white">"{{ $category->name }}"</strong>? Tindakan ini akan menghapus kategori secara permanen dari sistem.
                                            </flux:subheading>
                                        </div>
                                        ...
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <flux:button variant="danger" type="submit">Hapus Kategori</flux:button>
                                        </div>
                                    </form>
                                </flux:modal>
                            </flux:table.cell>
